...
LPVOID ProfileArgIterator::GetNextArgAddr()
{
    ...
    if (TransitionBlock::IsFloatArgumentRegisterOffset(argOffset))
    {
        ...
        if (pArgLocDesc)
        {
            ...
            else if ((pArgLocDesc->m_structFields & STRUCT_FLOAT_FIELD_ONLY_TWO) && !(pArgLocDesc->m_structFields & STRUCT_HAS_8BYTES_FIELDS_MASK))
            {
...
LPVOID ProfileArgIterator::GetReturnBufferAddr(void)
{
    ...
    if (fpReturnSize != 0)
    {
        if ((fpReturnSize & (UINT)STRUCT_FLOAT_FIELD_ONLY_TWO) && !(fpReturnSize & (UINT)STRUCT_HAS_8BYTES_FIELDS_MASK))
        {
            // For struct{single, single} case, fill and return the pData->buffer address.
        ...
        else if ((fpReturnSize & (UINT)STRUCT_FLOAT_FIELD_ONLY_ONE) || (fpReturnSize & (UINT)STRUCT_FLOAT_FIELD_ONLY_TWO))
        {
