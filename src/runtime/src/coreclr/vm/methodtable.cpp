...
// Return true if there are less than two fields in ELEMENT_TYPE_VALUETYPE.
static bool IsElementValueTypeLTTwoField(TypeHandle th)
{
    bool isManaged = !th.IsTypeDesc();
    bool ret = false;
    MethodTable* pMT = isManaged ? th.AsMethodTable() : th.AsNativeValueType();
    ...
    if ((th.GetSize() > 16 /*MAX_PASS_MULTIREG_BYTES*/) || (nFields > 1))
        return false;
    ...
    FieldDesc* fields   = pMT->GetApproxFieldDescListRaw();
    CorElementType type = fields[0].GetFieldType();

    if (type == ELEMENT_TYPE_VALUETYPE)
    {
        if (isManaged)
        {
            pMT = fields[0].GetApproxFieldTypeHandleThrowing().GetMethodTable();
            if (pMT->GetNumIntroducedInstanceFields() > 1)
            {
                return false;
            }
            return IsElementValueTypeLTTwoField(TypeHandle(pMT));
        }
        else // native layout
        {
            const NativeFieldDescriptor* nfields = pMT->GetNativeLayoutInfo()->GetNativeFieldDescriptors();
            NativeFieldCategory nfc = nfields->GetCategory();
            if (nfc == NativeFieldCategory::NESTED)
            {
                pMT = nfields->GetNestedNativeMethodTable();
                return IsElementValueTypeLTTwoField(TypeHandle(pMT));
            }
            else if (nfc != NativeFieldCategory::ILLEGAL)
            {
...
// Return 'RSFlag': return struct element flag
// bit0-bit7:  Struct's first  element type.
// bit8-bit15: Struct's second element type.
int MethodTable::GetLoongArch64ReturnStructElementType(MethodTable* pReturnTypeMT)
{
    _ASSERTE(pReturnTypeMT->IsRegPassedStruct());
    int RSFlag = 0;

    if (pReturnTypeMT->GetNumInstanceFieldBytes() > 16 /*MAX_PASS_MULTIREG_BYTES*/)
        return RSFlag;

    MethodTable* pMT     = nullptr;
    CorElementType type  = ELEMENT_TYPE_END;
    DWORD nFields = pReturnTypeMT->GetNumIntroducedInstanceFields();
    FieldDesc *pFieldFirst = pReturnTypeMT->GetApproxFieldDescListRaw();

    if (nFields == 1)
    {
        type = pFieldFirst[0].GetFieldType();

        if (type == ELEMENT_TYPE_VALUETYPE)
        {
            pMT  = pFieldFirst[0].GetApproxFieldTypeHandleThrowing().GetMethodTable();
            type = (CorElementType)GetLoongArch64ReturnStructElementType(pMT);
        }
    ...
    {
        FieldDesc *pFieldSecond = (pFieldFirst->GetOffset() == 0) ? pFieldFirst + 1 : pFieldFirst;
        pFieldFirst = (pFieldFirst->GetOffset() == 0) ? pFieldFirst : pFieldFirst + 1;

        assert(pFieldFirst->GetOffset() == 0);

        if ((pFieldFirst[0].GetSize() > 8) || (pFieldSecond[0].GetSize() > 8))
        {
            goto _End_arg;
        }
        ...
        {
            pMT  = pFieldFirst[0].GetApproxFieldTypeHandleThrowing().GetMethodTable();
            if (IsElementValueTypeLTTwoField(TypeHandle(pMT)))
            {
                type = (CorElementType)GetLoongArch64ReturnStructElementType(pMT);
            }
            else
        ...
        type = pFieldSecond[0].GetFieldType();

        if (type == ELEMENT_TYPE_VALUETYPE)
        {
            pMT  = pFieldSecond[0].GetApproxFieldTypeHandleThrowing().GetMethodTable();
            if (IsElementValueTypeLTTwoField(TypeHandle(pMT)))
            {
                type = (CorElementType)GetLoongArch64ReturnStructElementType(pMT);
            }
    ...
    {
        // We should recognize the GCType in return struct(size <=16 && nFields > 2) which is passed by multi reg.
        // eg: struct{string,float,int} , struct{int,int,class} , etc...
        // The GCReg can only be the first or the last element within the return struct in this case.
        if ((pFieldFirst[0].GetSize() == 8) || (pFieldFirst[nFields - 1].GetSize() == 8))
        {
            int index = (pFieldFirst[0].GetSize() == 8) ? 0 : nFields - 1;
            type = pFieldFirst[index].GetFieldType();

            if (type == ELEMENT_TYPE_VALUETYPE)
            {
                pMT = pFieldFirst[index].GetApproxFieldTypeHandleThrowing().GetMethodTable();
                if (IsElementValueTypeLTTwoField(TypeHandle(pMT)))
                {
                    type = (CorElementType)GetLoongArch64ReturnStructElementType(pMT);
                }
            ...
            // NOTE that the struct liking struct{int1,int2,class} passed by multi reg will be reordered.
            // That is, the `class` is passed by `$a0`, the `int1` and `int2` is passed by `$a1`.
            RSFlag = (int)type;
        }
        else
        {
